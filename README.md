...
- **指 2 本の長押しでサークルメニュー。** 何も選んでいなければカメラ（パース / 上 / 下 / 前 / 後 / 左 / 右 — Maya と同じ種類。パース以外は平行投影）、選んでいれば編集メニュー。北東の「新規カメラ」で今の視点に名前を付けて控えられ、輪の下の一覧から呼び戻せる
- **指 3 本でつまむと選択を拡大縮小。** マニピュレータを触らずに、選んでいる面 / エッジ / 頂点 / オブジェクトを直接。基点は選択の中心で、そこは動かない。**スケールだけ**で、回転も平行移動もしない（3 本指をそのまま滑らせても何も起きない）。ソフト選択と対称 X はそのまま効く
