...
### MovinkPad 14（Chrome 152、コア 8、画素比 1.75、100 万四角形）

| # | 何を | ms | 目標 |
|---|---|---|---|
| B0 | メッシュを作る（100 万四角形） | 4015 | |
| B1a | 細分割 1 レベル JS（25 万） | 3238 | |
| B1a-wasm | 細分割 1 レベル wasm（25 万） | **653** | |
| B1b | 細分割 1 レベル JS（100 万） | 16926 | 1500 |
| B1b-wasm | 細分割 1 レベル wasm（100 万） | **2637** | 1500 |
| 　 | うち wasm の計算 | 737 | |
| 　 | うち JS でメッシュに組む | 1900 | |
| B2a | レベル 2 まで組む（92 万） | 7869 | |
| B2b | **デルタの取り直し（1 万頂点）** | **150** | **16** |
| B3a | BVH を作る（199 万三角形） | 1387 | |
| B3b | BVH を取り直す（全部） | 64 | |
| B4 | 描画 1 フレーム | 0.3 | 16 |
| B5 | 使っているメモリ | **2222 MB** | |
...
**100 万四角形の途中で Safari が落ちた。** 上の B5 のとおり 2.2GB 使うので、
mini（メモリ 4GB、Safari のタブはその一部）では最後まで行かない。

### iPad Pro

ユーザーは持っていない。**ハイエンドの数字は MovinkPad 14 だけ**で決める。

---

## この数字で分かったこと（Opus）

### 1. B4 の 0.3ms は測れていなかった → 直した

`renderer.render` は GL に命令を積むだけで返る。199 万三角形が 0.3ms で
描けるはずがない。**CPU が命令を投げる時間しか測っていなかった。**

`readPixels` で積んだ命令を流し切らせてから測るように直した（B4a）。
実際に動かしたときのコマの間隔も足した（B4b）。CI の swiftshader で
0.3ms → 286ms になったので、いまは測れている。**B4 は測り直しが要る。**

### 2. 100 万四角形は 2.2GB。mini では届かない → 大きさを選べるようにした

`?size=25` / `?size=50` / `?size=100`（既定は 100）と、パネルのボタン。
mini はまず 25 万で測ってほしい。

なお 2.2GB の大半は `MeshBuilder` が使う JS の配列（`number[]`）。
100 万四角形を 1 レベル上げると 400 万四角形になり、コーナーは 1600 万個。
これを `number[]` で持つと 1 要素 8 バイト + 配列の余裕で、それだけで GB に届く。

### 3. B2b が目標の 9 倍。**ブラシは JS では届かない**

150ms に対して目標 16ms。指示書の判断の目安（`30` の T3）にそのまま当てると
**「B2 が入らないなら、ブラシの変形とデルタの取り直しも wasm」**になる。

### 4. wasm の効きは実機でも変わらない

25 万で 3238ms → 653ms（5.0 倍）。100 万で 16926ms → 2637ms（6.4 倍）。
wasm の計算だけなら 737ms なので、**残り 1900ms は JS の `MeshBuilder`**。
ここが次にいちばん大きい塊で、B0（4015ms）も B2a（7869ms）も同じ原因。

`MeshBuilder` を出す（あるいは、細分割のように出力が全部四角形と分かって
いる場面では `Mesh` を直に組む）と、B0・B1・B2a がまとめて速くなるはず。

**判断は Fable。** 上の 3 と 4 から、`31` では

- デルタの取り直しとブラシの変形を wasm へ（B2b が 16ms に入らないため）
- `MeshBuilder` を通さない組み立て（B0・B1・B2a とメモリにまとめて効く）
