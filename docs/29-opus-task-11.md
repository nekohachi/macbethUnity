...
> **2026-09-09 実機確認（ユーザー）:** ホーム画面から開いて回しても**潰れない**。上段のボタンも**ステータスバーの下に入っていない**。A-T2・A-T3 は実機でも通った。24px の保険は要らない。

---

## 実装の報告 その 2（Opus、2026-09-09。B-T4〜B-T6）

### 変えたファイル

**B-T4: 履歴の差分化**

- `app/history.ts`: 段を 3 種類に（`full` / `positions` / `transform`）。`beginPositions` / `beginTransform` / `commitPending` / `abortPending` / `upgradeToFull` / `lastEntry`。やり直しは差分を逆向きに当てるだけ（控えを取り直さない）。選択の戻しを `restoreSelection` に共通化
- `app.ts`: `beginDragHistory` / `commitDragHistory`（差分で済むか全複製かをここで決める）、`cornersAround`（Preserve UVs で書き換わるコーナー）
- `tests/history.test.ts`: 7 件

**B-T5: BVH**

- `core/bvh.ts`（新）: `buildBvh` / `raycastBvh` / `refitBvh` / `refitBvhPartial` / `trianglesNear`。平坦な配列で持ち、親と「三角形 → 葉」も持つ
- `render/meshView.ts`: `ObjectView.bvh`、`VertexSlots` と `buildVertexSlots`
- `render/viewport.ts`: `bvhOf`（初めて要るときに作る）、`refreshPositions` で `refitBvh`
- `render/picking.ts`: `pickSurface` を木で。`pickVertex` は当たった点のまわりの三角形だけを候補にする（4000 頂点未満は総当たりのまま。木を引く手間のほうが大きい）
- `tests/bvh.test.ts`: 6 件（総当たりと同じ答えを返すことが主）

**B-T6: 10 万三角形の門**

門を通すために 2 つ足した。**どちらも指示書に無いが、これが無いと門を越えられなかった**。

- `render/viewport.ts` の `refreshMoved`: ドラッグ中は**動いた頂点の分だけ**描画バッファを書き換える。`refreshPositions`（作り直し）は離したときに 1 回だけ
- `ui/hud.ts`: ポリゴンカウントの控え。数を出すにはエッジを数え直す必要があり、10 万三角形で 1 回 40ms かかる。**トポロジが変わっていなければ前の数を使う**

### 数

| | `29` の前 | A-T3 まで | B-T6 まで |
|---|---|---|---|
| core の単体（vitest） | 210 | 210 | 223 |
| 通し確認（smoke） | 101 | 104 | 106 |

PNG: `docs/img/29-t6-100k.png`（10 万三角形の球。カウントが写っている）。公開版（`app/`）も更新済み。

### 門の実測（CI の Chromium。swiftshader なので実機はこれより速い）

| 測ったもの | 直す前 | 直した後 | 上限（実測の 1.5 倍に締めた） |
|---|---|---|---|
| `pickSurface` 200 回 | — | **82〜87ms** | 130ms |
| ホバー 60 回（プリセレクション） | — | **387〜413ms** | 600ms |
| **ツイーク 30 コマ** | **5258ms**（175ms / コマ） | **392〜455ms**（14ms / コマ） | 700ms |
| `frameSelected` | — | 10〜38ms | 60ms |
| 取り消し → やり直し | — | **0〜1ms** | 60ms |
| ドラッグ 1 段の履歴 | 2.5MB（全複製） | **832 バイト** | 8KB |

**ツイークが 12 倍速くなった。** 内訳は、描画バッファの作り直しをやめたのが 80ms / コマ、ポリゴンカウントの数え直しをやめたのが 80ms / コマ。

### 設計と変えたところ

1. **ドラッグ中は法線を据え置きます**（`refreshMoved`）。動かしている間だけ陰影が少し古く、離した瞬間に直ります。作り直すと 10 万三角形で 80ms / コマかかり、門を越えられません。ワイヤと頂点の位置は毎コマ正しく動きます
2. **BVH に「一部だけ取り直す」を足しました**（`refitBvhPartial`）。全部取り直すと 10 万三角形で 40ms かかります。触れた葉とその先祖だけなら 1ms 以下。**スカルプトのストロークもこれを使います**
3. **ターゲットウェルドは差分から全複製へ格上げします**（`upgradeToFull`）。ドラッグの途中でトポロジが変わると分かるので、動かす前の座標へ一度戻して控えを取り、動かし直してから溶接します
4. **Preserve UVs でレシピを持つオブジェクトは全複製のままです。** レシピへの差分（`recordManual`）は座標の差分に入らないためです。レシピが無い（`map1` を直に書く）ときは、書き換わるコーナーの UV も差分に入れています
5. 通し確認のツイークは**面**で見ています。頂点 1 つだと近くの頂点へ溶接されてトポロジが変わり、全複製になるためです（それは正しい動き）

### 残っている借金（スカルプトの前にもう一度見る）

- **`mesh.vertexFaces()` が毎回 O(コーナー数)**（10 万三角形で 11ms）。`preserveUvs` がドラッグのたびに呼びます。メモ化できますが、面の配列をその場で書き換えるコードが無いことを確かめてからにしたいので、S2 で測ってから
- **`beginPreserve` が座標と UV を丸ごと複製**します（10 万頂点で 1.2MB / ドラッグ）。`recordPreserved` が全コーナーを走査するので、そこも一緒に絞る必要があります
- どちらも**モデリングの規模では困りません**（門は通りました）。スカルプトのストロークで効いてきます

### 次

`28` の S2-0（測る）と wasm の toolchain。指示書は `30`。
