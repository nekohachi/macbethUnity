...
## 実装の報告 その 4（Opus、2026-09-08。フィードバック 4 点 + T5・T6・T7・T9）

**core 196 件、通し確認 73 項目、すべて緑。指示書 `20` のタスクはこれで全部終わり。**

### タブレットで見つかった 4 点

| 指摘 | 原因 | 直したこと |
|---|---|---|
| UV モードにもプリミティブとカメラのアイコンが欲しい | — | UV のツール列に「3D」の区切りを作って、モデリングと同じ 2 グループを置いた |
| UV モードで 3D ビューを矩形選択すると、枠が 2D 側に出る | 枠は `#vp` に置いてあるのに、座標は `#pane3d` の中のもの。分割表示だと 3D ペインは右半分から始まるので、ちょうど半画面ぶん左にずれて 2D の上に出ていた | 2D と同じようにペインの位置ぶんずらす |
| 分割数を変えると UV が壊れる | 作り直すと頂点の番号が全部変わるので、切れ目もピンも差分も別のところを指す | `rebuildRecipeFor` を足した。**設定（方式・パッキング・自動の設定）だけ残して、新しいメッシュの UV から切れ目を取り直す**。分割を変えても UV はレシピから作り直される |
| 球で SHF の面選択が全然できない | **SHF + ダブルクリックがシェル全体を選んでいた。** 細かい球だと隣の面を続けて叩くだけで「同じ場所を 2 回」に入り、選択が毎回シェル全部に化けていた | (1) ダブルクリックの条件に**「同じコンポーネントを 2 回」**を足した（3D も 2D も）。(2) **足しているとき（SHF / CTL）はフェースをシェルに広げない**。(3) カメラベース選択の**控えが古くなる**問題も直した（カメラを回しても作り直さないので、回したあとは前の向きで判定していた）。(4) `pickSurface` も裏を向いた面を飛ばすようにした |

### T5: 手順 B（円柱）

![T5 円柱の帯を格子に](img/20-t5-cylinder.png)

- `core/uv/loops.ts` に **`uvStraightRun`**（格子に沿ってまっすぐ進む）と **`uvGridRows`**（島が四角形の帯なら行を返す）
- 2D に **「境界の直線化」**（エッジ単位）と **「格子化」**（シェル単位）
- **`applyToUvVertices`**: 並びが意味を持つ操作は、UV 頂点ごとの配列に写してから当てて、コーナーへ書き戻す。同じ UV に何本ものコーナーが重なるので、そのまま渡すと並びが壊れる

### T6: 手順 C（球、Move and Sew）

- `core/uv/ops.ts` に **`similarityFrom2`**（2 点の対応から移動 + 回転 + 一様スケール。複素数の割り算 1 回で出る）と `applySimilarity`
- 2D に **`moveAndSew()`**。切れ目の両端が相手側に重なるよう**小さいほうの島**を寄せてから縫う。「ソー」はエッジ / シェルではこちらになった（頂点単位は今までどおり）

### T7: 自動 UV を奥へ

- ツール列から「自動 UV」のボタンを外し、**「展開」の長押し**（展開 / 自動 UV / 整列 / オプション…）へ
- カットインでは方式と自動 UV を**折りたたみ**（既定は閉じる）にして、パッキングだけ表に出した
- シェルの編集メニューから自動 UV を外し、空いた南東に 90° 回転を入れた

### T9: アウトライナ → レイヤー

![T9 レイヤーのドロワー](img/20-t9-layers.png)

- **狭い画面（1200px 未満）は右から被さるドロワー**。ビューポートは縮まない。右端から左へのスワイプでも出る
- **広い画面はドッキングのまま**
- 行はサムネイル / 名前 / 目 / ロック / 「>」。「>」でトランスフォームの数値とメッシュの情報が開く
- 長押しでメニュー、**長押しのままドラッグで並び替え**、ダブルタップで改名
- `SceneObject.locked` を core に足した（履歴と `.mbz` に残り、ロック中は選べない）
- サムネイルは、そのオブジェクトだけを描いた 80px の画像。形が変わるまで使い回す

### 設計と変えたところ

- **`applyToUvVertices` を足した（指示書に無い）。** `ops.ts` の関数は「点の並び」で受けるが、`straightenBorder` と `gridding` は並びが意味を持つ。コーナーをそのまま渡すと、同じ UV に重なるコーナーが並びに混ざって向きの判定が壊れる
- **T7 のメニュー変更を T5 のときに前倒しした。** シェルの編集メニューは 8 方位が埋まっていて、格子化を入れる空きが無かった
- **サムネイルは 80px で描いて 34px で見せる。** 34px で描くと粗すぎて何か分からない
- **並び替えは履歴に積んでいない。** 表示の順だけで、メッシュにもレシピにも影響しないため

### 通し確認に足した項目（5 つ、73 項目に）

フィードバック 4 点 / 手順 B（円柱）/ T7（自動 UV の位置）/ 手順 C（球）/ レイヤーのドロワー。
